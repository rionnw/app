...
namespace face {
	...
	int mod(int a, int m) { return a > 0 ? a % m : (a % m + m) % m; }
	...
	int encode(const std::string& cubelet, int ori) {
		...
		for (int i = 0; i < cubelet.size(); i++)
			res = color::COUNT * res +
			color::FROM_NAME.at(cubelet[mod(i - ori, cubelet.size())]);
		return res;
	}
	...
	void init() {
		for (int corner = 0; corner < cubie::corner::COUNT; corner++) {
			for (int ori = 0; ori < 3; ori++)
				corners[encode(cubie::corner::NAMES[corner], ori)] =
				std::make_pair(corner, ori);
		}
	...
	std::string from_cubie(const cubie::cube& c) {
		...
		for (int corner = 0; corner < cubie::corner::COUNT; corner++) {
			for (int i = 0; i < 3; i++)
				...
				s[CORNLETS[corner][i]] =
				cubie::corner::NAMES[c.cperm[corner]][mod(i - c.cori[corner], 3)];
		}
		for (int edge = 0; edge < cubie::edge::COUNT; edge++) {
			for (int i = 0; i < 2; i++)
				s[EDGELETS[edge][i]] =
				cubie::edge::NAMES[c.eperm[edge]][mod(i - c.eori[edge], 2)];
		}
