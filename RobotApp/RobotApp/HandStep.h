...
inline bool cmp(const searchres& s1, const searchres& s2)
{
...
class Engine
{
    ...
    int max_len;     // find solutions with at most this length; -1 means simply search for the full
                     // `tlimit`
    ...
    std::priority_queue<searchres, std::vector<searchres>, decltype(&cmp)> sols{
        cmp};                           // already found solutions
...
public:
    Engine(int n_threads, int tlim, int n_sols = 1, int max_len = -1, int n_splits = 1);
