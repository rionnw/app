...
#### 第三轮（删 MechanicalStep::time + cost 提取）

- 移除 C 端遗留的 `MechanicalStep::{time, name}`：在 RobotApp 全工程
  内只写不读，DFS 用的是 `MechanicalGroup::time`（整组累加值）。
  Rust 端把 `MechanicalStep` 退化成 `pub type StepNum = i8`，
  `MechanicalGroup.steps` 从 `[MechanicalStep; 20]`（160 字节）
  变 `[i8; 20]`（20 字节）。
- DFS 内 `g_mov_buff[base + _i] = group.steps[_i].num` 的 `for` 循环
  改为 `copy_from_slice` 一次（编译器友好的 memcpy）。
- `Time_Air / Tim_*` 提取为 `pub struct CostModel`，可注入；
  `pub fn build_mech_lib_with(cost)` 暴露给上层"切换 cost 后重建
  操作库"。
...
| 输入                       | 第二轮后 | 第三轮后 | 提升  |
| -------------------------- | -------- | -------- | ----- |
| perf_len1                  | 563 ns   | 286 ns   | 2.0×  |
| perf_len5                  | 45 µs    | 31 µs    | 1.5×  |
| perf_len10                 | 198 µs   | 178 µs   | 1.1×  |
| 真实 20 face Kociemba 解   | 3.24 ms  | 1.00 ms  | 3.2×  |
| 真实 21 face Kociemba 解   | 2.63 ms  | 0.93 ms  | 2.8×  |

**真实 Kociemba 解 ~2.5× 提速**——原因是长输入 DFS 节点 2K-6K，
`MechanicalGroup` 缩小 50% 让更多 group 能进 L1/L2 cache。

baseline 全部 byte-equivalent，DFS 节点数完全相同 → 语义零变化。
