...
private:
    void phase1(int depth, int togo, int flip, int slice, int twist, int corners, move::mask next,
                move::mask qt_skip);   // phase 1 search; iterates through all solution with exactly
                                       // `togo` moves
    bool phase2(int depth, int togo, int slice, int udedges2, int corners, move::mask next,
                move::mask qt_skip);   // phase 2 search; returns once any solution is found

public:
    Search(int dir, const coordc& cube, int p1depth, move::mask d0moves, bool& done, int& lenlim,
           Engine& solver)
        : dir(dir)
        , cube(cube)
        , p1depth(p1depth)
...
void Search::run()
{
    ...
    next &=
        move::p1mask & d0moves;   // block B-moves in F5 mode here and select current search split
    phase1(0, p1depth, cube.flip, cube.slice, cube.twist, cube.corners, next, 0);
}

void Search::phase1(int depth, int togo, int flip, int slice, int twist, int corners,
                    move::mask next, move::mask qt_skip)
{
    if (done) return;
    if (togo == 0) {
...
#ifndef AX
#    ifdef QT
        delta++;   // in vanilla QT mode the perm-parity indicates whether solution length is odd or
                   // even
#    endif
#endif
        for (int togo1 = std::max(prun::get_phase2(corners, udedges2), tmp); togo1 < lenlim - depth;
             togo1 += delta) {
            if (phase2(depth,
                       togo1,
                       slice,
                       udedges2,
                       corners,
                       move::p2mask & move::next_p1p2[moves[depth - 1]],
                       qt_skip))
                return;   // once we have found a phase 2 solution, there cannot be any shorter ones
                          // -> quit
        }
        return;
    }
    ...
    while (next) {
        int m =
            cubie::ffsll(next) - 1;   // get rightmost move index (`ffsll()` uses 1-based indexing)
        next &= next - 1;
    ...
    // We always want to maintain the maximum number of already reconstructed EDGES coordinates,
    // hence we only decrement when the depth level gets lower than the current valid index (note
    // that we will typically also visit other deeper branches in between that might not have any
    // effect on this)
    if (edges_depth == depth - 1) edges_depth--;
}

bool Search::phase2(int depth, int togo, int slice, int udedges2, int corners, move::mask next,
                    move::mask qt_skip)
{
    if (togo == 0) {
        ...
        for (int i = 0; i < depth; i++) sol.first[i] = moves[i];
...
#ifdef QT
            // As we never want to leave the set of phase 2 cubes (which we would by doing only a
            // quarter-turn on an axis for which only double-moves are permitted), we need special
            // handling of the double moves. The simplest way to do this is to treat a double moves
            // simply as if two consecutive quarter-turns were added to the current search path.
            if (m >= move::COUNT1) {
                if (togo <=
                    1)   // we cannot do half turns when only a single quarter-turn is permitted
                    break;
...
Engine::Engine(int n_threads, int tlim, int n_sols, int max_len, int n_splits)
    ...
    , n_splits(n_splits)
{
    int tmp = (move::COUNT1 + n_splits - 1) /
              n_splits;   // ceil to make sure that we always include all moves
    for (int i = 0; i < n_splits; i++) masks[i] = (move::mask(1) << tmp) - 1 << tmp * i;
...
void Engine::thread()
{
    ...
    do {
        ...
        for (int dir = 0; dir < N_DIRS; dir++) {
            if (depths[dir] < depths[mindir]) mindir = dir;
        }
...
void Engine::prepare()
{
    ...
    for (int i = 0; i < n_threads; i++) threads.push_back(std::thread([&]() { this->thread(); }));

    done   = false;
    lenlim = max_len > 0 ? max_len + 1 : 50;   // only search for strictly shorter solutions than
                                               // this `sols` is always emptied after a solve
}
...
void Engine::solve(const cubie::cube& c, std::vector<std::vector<int>>& res)
{
    ...
    for (int dir = 0; dir < N_DIRS; dir++) {
        const cubie::cube& c1 =
            (dir & 1) ? invc : c;   // reference is enough, we do not need to copy
    ...
    {   // timeout
        ...
        if (!done) done = true;   // if we get here, this was a timeout
    }
    std::lock_guard<std::mutex> lock(
        sol_mtx);   // make sure no thread is writing any more solutions
    ...
    for (int i = 0; i < res.size(); i++) {
        ...
        if (sol.second & 1) {   // undo inversion
            for (int j = 0; j < res[i].size(); j++) res[i][j] = move::inv[res[i][j]];
...
void Engine::report_sol(searchres& sol)
{
    ...
    if (done)   // prevent any type of reporting after the solver has terminated (important for
                // threading)
        return;

    sols.push(sol);   // usually we only get here if we actually have a solution that will be added
    if (sols.size() > n_sols) sols.pop();
    if (sols.size() == n_sols) {
